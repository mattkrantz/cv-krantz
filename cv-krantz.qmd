...
2026 (anticipated) & MS & Vanderbilt University, Nashville, TN\\
 & & \hspace{2em}Biomedical Informatics\\[5pt]
\end{longtable}
...
research_supervision %>% dplyr::filter(category=="phd-student") %>%
  pull(citation) %>% 
  make_bullet_list()
```
...
research_supervision %>% filter(category=="clinical-fellow") %>%
  pull(citation) %>% 
  make_bullet_list()
```
...
```{r}
#| results: asis
grants %>% filter(pi==1) %>% pull(citation) %>% make_ordered_list()
```
...
```{r}
#| results: asis
grants %>% filter(co_investigator==1) %>% pull(citation) %>% make_ordered_list()
```
